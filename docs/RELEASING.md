...
The package is `@avant-garde/mastra-multiplayer`, and **a scoped package
...
Publishing also needs membership with publish rights in the existing
`avant-garde` npm organization. The npm scope intentionally differs from the
`Avant-Garde-AI` GitHub organization name.
...
```bash
npm pack
cd $(mktemp -d) && npm init -y > /dev/null && npm i /path/to/avant-garde-mastra-multiplayer-*.tgz
node -e "import('@avant-garde/mastra-multiplayer').then(m => console.log(Object.keys(m).length, 'exports'))"
```
